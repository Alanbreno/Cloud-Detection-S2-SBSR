...
tb_logger = TensorBoardLogger(config.DIR_LOG, name=config.NAME_EFFICIENTNETB2)
...
# Define the model
model = UNet_S2_Br(
    encoder_name=config.ENCODER_NAME_EFFICIENTNETB2,
...
checkpoint_callback = pl.callbacks.ModelCheckpoint(
    dirpath=config.DIR_ROOT_EFFICIENTNETB2,
...
trainer = pl.Trainer(
    ...
    precision="16-mixed",
    logger=tb_logger,
    default_root_dir=config.DIR_ROOT_EFFICIENTNETB2,
)
...
smp_model.save_pretrained(config.DIR_ROOT_EFFICIENTNETB2 + "/" + config.NAME_EFFICIENTNETB2)
